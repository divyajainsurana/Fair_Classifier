...
#    ansz1=ansz1[:,np.newaxis]
    ...
    logprobs = (np.multiply(-np.log(A3),Y) + np.multiply(-np.log(1 - A3), 1 - Y))+1*abs(ans2/ans1-ans1/ans2)**2+abs(ansz2/ansz1-ansz1/ansz2)**2#0.2*abs(count/len(X1) - count2/len(X2))#,ans2[0]/ans1[0])
...
def backward_propagation(parameters, cache, X, Y,z1,z2):
   ...
   # print (m)
    ...
    mat = np.dot(np.transpose(C22),sigmoidPrime(C31))
    sat = np.dot(np.transpose(D22),sigmoidPrime(D31))
    #print (cat.shape,zat.shape)
        
    dC3 =1/m*(cat -  zat+mat-sat)
    cat2 = np.dot(np.transpose(X12),sigmoidPrime(X21))
    zat2 = np.dot(np.transpose(B12),sigmoidPrime(B21))
    mat2 =np.dot(np.transpose(C12),sigmoidPrime(C21))
    sat2 = np.dot(np.transpose(D12),sigmoidPrime(D21))
    #print (cat2.shape,zat2.shape)
...
parameters = nn_model(X, Y,z_train1,z_train2, n_h = 16, num_iterations = 5000, print_cost=True)
