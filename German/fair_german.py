...
def compute_cost(A2, Y, parameters,z):
    ...
    z=z[:,np.newaxis]
    covariance = cov(z,A2)
    logprobs = 0.8*np.multiply(-np.log(A2),Y) + np.multiply(-np.log(1 - A2), 1 - Y)+0.2*(covariance**2)
...
def backward_propagation(parameters, cache, X, Y,z):
   ...
   # print (m)
    ...
    z=z[:,np.newaxis]
    mat = np.full((800,9),z)
    dC2 =(1/m*np.dot(np.transpose(A2),z))
...
def update_parameters(parameters, grads, covariance,learning_rate = 3):
    """
