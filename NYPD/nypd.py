...
z_test = xtest[0:,13]
z_train= X[0:,13]
print (z_test[8],z_train[2],X.shape,xtest.shape)
# Visualize the data:
...
def compute_cost(A2, Y, parameters,z):
    ...
    z=z[:,np.newaxis]
    covariance = cov(z,A2)
    logprobs = 0.4*(np.multiply(-np.log(A2),Y) + np.multiply(-np.log(1 - A2), 1 - Y))+0.6*(covariance**2)
...
def backward_propagation(parameters, cache, X, Y,z):
 ...
 #   print (covariance)
  ...
  #  db1 = 1/m * np.sum(dZ1,axis =1, keepdims =True)
   # dC2 = sigmoidPrime(1/m*np.dot(np.transpose(A1),z))
    #mat = np.full((24129,14),z)
    #dC2 =(1/m*np.dot(np.transpose(A1),z))
    #dC1 =(1/m*np.dot(np.transpose(X),mat))
    #dC1 = 1/m*np.dot(a2,np.transpose(a2))
    #print (dC2.shape, dC1.shape,dW1.shape)
    grads = {"dW1": dW1,
             "dC2": dC2,
             "dW2": dW2,
             "dC1": dC1,
...
def update_parameters(parameters, grads, covariance,learning_rate = 0.1):
...
for i in range(len(z_test)):
    #print (z_test[i])
    if z_test[i]==-0.3963992115132026  :
        count_f+=1
    else:
        count_m+=1
...
#    print (i,j)
    if j==-0.3963992115132026   and i ==1:
        prob_f+=1
      #  print(j,i)-4.66909330348592
    elif j==2.522709357020741 and i==1:
        prob_m+=1
prob_f_dm,prob_m_dm=0,0
for i,j,k in zip(out,z_test,ytest):
    if j==-0.3963992115132026 and i ==1 and k==0:
        prob_f_dm+=1
      #  print(j,i)
    elif j==2.522709357020741 and i==1 and k==0:
        prob_m_dm+=1
prob_f_dm_n,prob_m_dm_n=0,0
for i,j,k in zip(out,z_test,ytest):
    if j==-0.3963992115132026 and i ==0 and k==1:
        prob_f_dm_n+=1
      #  print(j,i)
    elif j==2.522709357020741 and i==0 and k==1:
        prob_m_dm_n+=1
